...
function get_one_objet($id)
{
    $bdd = dbconnect();
    $sql = sprintf("SELECT * FROM v_emprunt_objet_cate WHERE id_objet = '%s'", $id);
    $resultat = mysqli_query($bdd, $sql);
    return $resultat;

}

function get_emprunt($idm)
{
    $bdd = dbconnect();
    $sql = sprintf("SELECT * FROM v_emprunt_emprunt WHERE id_membre = '%s'", $idm);
    $resultat = mysqli_query($bdd, $sql);
    return $resultat;
